 ...
 */
public final class VectoredRangeReadUtils {
   ...
   */
  public static boolean isOrderedDisjoint(List<? extends VectorFileRange> input,
                                          int chunkSize,
                                          int minimumSeek) {
    long previous = -minimumSeek;
    for (VectorFileRange range: input) {
  ...
  public static List<? extends VectorFileRange> validateNonOverlappingAndReturnSortedRanges(
          List<? extends VectorFileRange> input) {
   ...
   * @return the list of sorted CombinedVectorFileRanges that cover the input
   */
  public static List<CombinedVectorFileRange> mergeSortedRanges(List<? extends VectorFileRange> sortedRanges,
                                                          int chunkSize,
                                                          int minimumSeek,
                                                          int maxSize) {

    CombinedVectorFileRange current = null;
    List<CombinedVectorFileRange> result = new ArrayList<>(sortedRanges.size());

    // now merge together the ones that merge
    for (VectorFileRange range: sortedRanges) {
      ...
      if (current == null || !current.merge(start, end, range, minimumSeek, maxSize)) {
        current = new CombinedVectorFileRange(start, end, range);
        result.add(current);
      }
    }
    return result;
   ...
   * private constructor.
   */
  private VectoredRangeReadUtils() {
